...
function checkJump(heights) {
    const peak = Math.max.apply(Math, heights);
    ...
    const asc = before.length > 0;
    const desc = after.length > 0;
    const ascSorted = before.slice(1).every(function (h, i) { 
      return h >= before[i]; });
    const descSorted = after.slice(1).every(function (h, i) { 
      return h <= after[i]; });
    return asc && desc && ascSorted && descSorted;
}
